...
const CourseLayout: FC<CourseLayoutProps> = ({ title, date, slug, author, tags, description = "", source }) => {
  const keywords = tags.map((it) => getTag(it)?.name).filter(Boolean);
  const authorName = getAuthor(author)?.name;
  return (
    <Layout>
      <BasicMeta url={`/courses/${slug}`} title={title} keywords={keywords} description={description} />
      <TwitterCardMeta url={`/courses/${slug}`} title={title} description={description} />
      <OpenGraphMeta url={`/courses/${slug}`} title={title} description={description} />
      <JsonLdMeta
        url={`/courses/${slug}`}
        title={title}
        keywords={keywords}
